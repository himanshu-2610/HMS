...
app.get('/patient/change-password', requirePatient, (req, res) => {
    res.render('patient/change-password', { 
        error: null,
        user: {
            ...req.user, // Spread existing user properties
            role: 'patient' // Explicitly add role
        },
        activePage: 'profile'
    });
});
...
app.post('/patient/change-password', requirePatient, async (req, res) => {
    ...
    try {
        ...
        // Update password (you should really hash passwords in production!)
...
app.get('/admin/messages', requireAdmin, async (req, res) => {
        ...
        const [messages] = await pool.query(`
            SELECT m.*, 
                   ...
                   END as sender_name,
                   CASE 
                       WHEN m.receiver_type = 'patient' THEN pt.full_name
                       WHEN m.receiver_type = 'doctor' THEN dt.full_name
            ...
            LEFT JOIN patients pt ON m.receiver_type = 'patient' AND m.receiver_id = pt.patient_id
            LEFT JOIN doctors dt ON m.receiver_type = 'doctor' AND m.receiver_id = dt.doctor_id
            LEFT JOIN admins at ON m.receiver_type = 'admin' AND m.receiver_id = at.admin_id
            WHERE (m.receiver_type = 'admin' AND m.receiver_id = ?) OR 
                (m.sender_type = 'admin' AND m.sender_id = ?)
            ORDER BY m.created_at DESC
        `, [req.session.user.id, req.session.user.id]);

        // Get all active patients and doctors for dropdowns
        const [patients] = await pool.query('SELECT patient_id, full_name FROM patients ');
        const [doctors] = await pool.query('SELECT doctor_id, full_name FROM doctors ');
...
app.post('/admin/messages/send', requireAdmin, async (req, res) => {
    ...
    try {
        ...
        await pool.query(
            ...
            ['admin', req.session.user.id, receiver_type, receiver_id, subject, message]
        );
        pool.release();
...
app.get('/doctor/messages', requireDoctor, async (req, res) => {
        ...
        const [messages] = await pool.query(`
            ...
            LEFT JOIN patients pt ON m.receiver_type = 'patient' AND m.receiver_id = pt.patient_id
            LEFT JOIN doctors dt ON m.receiver_type = 'doctor' AND m.receiver_id = dt.doctor_id
            WHERE (m.receiver_type = 'doctor' AND m.receiver_id = ?) OR 
...
app.get('/patient/messages', requirePatient, async (req, res) => {
    try {
        ...
        const [messages] = await pool.query(`
            ...
            LEFT JOIN patients pt ON m.receiver_type = 'patient' AND m.receiver_id = pt.patient_id
            LEFT JOIN doctors dt ON m.receiver_type = 'doctor' AND m.receiver_id = dt.doctor_id
            WHERE (m.receiver_type = 'patient' AND m.receiver_id = ?) OR 
                  (m.sender_type = 'patient' AND m.sender_id = ?)
            ORDER BY m.created_at DESC
        `, [req.session.user.id, req.session.user.id]);

        // Get doctors for the message form
        ...
        `, [req.session.user.id]);
        const [admins] = await pool.query('SELECT admin_id, full_name FROM admins ');
...
app.get('/:role/messages/:id', async (req, res) => {
    ...
    try {
        ...
        if (message.length > 0 && !message[0].is_read && message[0].receiver_id == req.session.user?.id) {
            await pool.query('UPDATE messages SET is_read = TRUE WHERE message_id = ?', [id]);
        }
...
app.post('/:role/messages/reply/:id', async (req, res) => {
    const { role, id } = req.params;
    ...
    try {
        ...
        const [original] = await pool.query('SELECT * FROM messages WHERE message_id = ?', [id]);
        ...
        await pool.query(
            'INSERT INTO messages (sender_type, sender_id, receiver_type, receiver_id, subject, message) VALUES (?, ?, ?, ?, ?, ?)',
            [role, req.session.user.id, originalMessage.sender_type, originalMessage.sender_id, subject, message]
        );
...
app.post('/:role/messages/:id/read', async (req, res) => {
    const { role, id } = req.params;
