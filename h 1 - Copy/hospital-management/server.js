...
app.get('/admin/messages', requireAdmin, async (req, res) => {
    try {
        ...
        // Get list of patients and doctors for the message form
...
app.get('/admin/messages/:id', requireAdmin, async (req, res) => {
    try {
        const [messages] = await db.query(`
            ...
            LEFT JOIN patients p ON m.sender_type = 'patient' AND m.sender_id = p.patient_id
            LEFT JOIN doctors d ON m.sender_type = 'doctor' AND m.sender_id = d.doctor_id
            WHERE m.message_id = ? AND (m.receiver_type = 'admin' AND m.receiver_id = ? OR m.sender_type = 'admin' AND m.sender_id = ?)
        ...
        if (messages[0].receiver_type === 'admin' && messages[0].receiver_id === req.session.user.id) {
...
app.get('/doctor/messages', requireDoctor, async (req, res) => {
    try {
        ...
        // Get list of patients for the message form
        const [patients] = await db.query(`
...
app.get('/doctor/messages/:id', requireDoctor, async (req, res) => {
    try {
        const [messages] = await db.query(`
            SELECT m.*, 
                   CASE 
            ...
            LEFT JOIN patients p ON m.sender_type = 'patient' AND m.sender_id = p.patient_id
            LEFT JOIN doctors d ON m.sender_type = 'doctor' AND m.sender_id = d.doctor_id
            WHERE m.message_id = ? AND (m.receiver_type = 'doctor' AND m.receiver_id = ? OR m.sender_type = 'doctor' AND m.sender_id = ?)
        `, [req.params.id, req.session.user.id, req.session.user.id]);
        ...
        // Mark message as read if it's an incoming message
        if (messages[0].receiver_type === 'doctor' && messages[0].receiver_id === req.session.user.id) {
            await db.query('UPDATE messages SET is_read = TRUE WHERE message_id = ?', [req.params.id]);
        }
...
// Add new route to fetch a single message for patients
app.get('/patient/messages/:id', requirePatient, async (req, res) => {
    try {
        const [messages] = await db.query(`
            SELECT m.*, 
                   CASE 
            ...
            LEFT JOIN patients p ON m.sender_type = 'patient' AND m.sender_id = p.patient_id
            LEFT JOIN doctors d ON m.sender_type = 'doctor' AND m.sender_id = d.doctor_id
            WHERE m.message_id = ? AND (m.receiver_type = 'patient' AND m.receiver_id = ? OR m.sender_type = 'patient' AND m.sender_id = ?)
        `, [req.params.id, req.session.user.id, req.session.user.id]);
        ...
        // Mark message as read if it's an incoming message
        if (messages[0].receiver_type === 'patient' && messages[0].receiver_id === req.session.user.id) {
            await db.query('UPDATE messages SET is_read = TRUE WHERE message_id = ?', [req.params.id]);
        }
...
app.get('/:role/messages/unread-count', requireAuth, async (req, res) => {
    try {
      const [result] = await db.query(
        'SELECT COUNT(*) as count FROM messages WHERE receiver_type = ? AND receiver_id = ? AND is_read = FALSE',
  ...
  // Update all existing message routes to use the new query
  app.get('/:role/messages', requireAuth, async (req, res) => {
    try {
      const messages = await getMessagesForUser(req.session.user);
      
      // Get additional data needed for the form
      if (req.session.user.role === 'admin') {
        const [patients] = await db.query('SELECT patient_id, full_name FROM patients');
        const [doctors] = await db.query('SELECT doctor_id, full_name FROM doctors');
        return res.render(`${req.session.user.role}/messages`, { 
          user: req.session.user, 
          messages,
          patients,
          doctors
        ...
        const [admins] = await db.query(`
          SELECT admin_id as id, full_name, 'admin' as role FROM admins 
          UNION 
          SELECT doctor_id as id, full_name, 'doctor' as role FROM doctors WHERE is_active = TRUE
        `);
        return res.render(`${req.session.user.role}/messages`, { 
          user: req.session.user, 
          messages,
  ...
  async function getMessagesForUser(user) {
    const query = `
      SELECT m.*, 
             ...
             END as sender_name,
             CASE 
                 WHEN m.receiver_type = 'patient' THEN p2.full_name
                 WHEN m.receiver_type = 'doctor' THEN d2.full_name
                 ELSE 'Admin'
             END as receiver_name
      FROM messages m
      LEFT JOIN patients p ON m.sender_type = 'patient' AND m.sender_id = p.patient_id
      LEFT JOIN doctors d ON m.sender_type = 'doctor' AND m.sender_id = d.doctor_id
      LEFT JOIN patients p2 ON m.receiver_type = 'patient' AND m.receiver_id = p2.patient_id
      LEFT JOIN doctors d2 ON m.receiver_type = 'doctor' AND m.receiver_id = d2.doctor_id
      WHERE (m.receiver_type = ? AND m.receiver_id = ?) 
         OR (m.sender_type = ? AND m.sender_id = ?)
      ORDER BY m.created_at DESC
    `;
